...
import com.nextos.eplayer.BuildConfig
import com.nextos.eplayer.R
import com.nextos.eplayer.media.utils.Preferences
...
object VLCOptions {
    ...
    val libOptions: ArrayList<String>
        get() {
            ...
            val timeStrechingDefault =
                context.resources.getBoolean(R.bool.player_time_stretching_default)
            val timeStreching =
                pref.getBoolean("enable_time_stretching_audio", timeStrechingDefault)
            ...
            // "subtitles_color" type changed
            var subclrStr: String? = null
            try {
                subclrStr = pref.getString("subtitles_color", "16777215")
            } catch (e: Exception) {
            }
            var subclrInt = 16777215

            if (subclrStr != null) {
                subclrInt = Integer.valueOf(subclrStr)
            } else {
                subclrInt = pref.getInt("subtitles_color", 16777215)
            }

            val freetypeColor = Integer.decode(String.format("0x%06X", (0xFFFFFF and subclrInt)))
            val freetypeColorOpacity = pref.getInt("subtitles_color_opacity", 255)

            val freetypeBackgroundColor = Integer.decode(
                String.format(
                    "0x%06X", (0xFFFFFF and pref.getInt("subtitles_background_color", 16777215))
                )
            )
            ...
            val freetypeOutlineColor = Integer.decode(
                String.format(
                    "0x%06X", (0xFFFFFF and pref.getInt("subtitles_outline_color", 0))
                )
            )
            ...
            val freetypeShadowColor = Integer.decode(
                String.format(
                    "0x%06X", (0xFFFFFF and pref.getInt(
                        "subtitles_shadow_color", Color.parseColor("#000000")
                    ))
                )
            )
            ...
            if (pref.getBoolean("prefer_smbv1", true)) options.add("--smb-force-v1")
            ...
            if (pref.getBoolean("audio-replay-gain-enable", false)) {
                ...
                else options.add("--no-audio-replay-gain-peak-protection")
            }
            ...
            if (BuildConfig.DEBUG) Log.d(
                this::class.java.simpleName, "VLC Options: ${options.joinToString(" ")}"
            )
            return options
        }

    fun isAudioDigitalOutputEnabled(pref: SharedPreferences) =
        pref.getBoolean("audio_digital_output", true)
    ...
    fun getAout(pref: SharedPreferences): String? {
        ...
        if (hwaout == HWDecoderUtil.AudioOutput.AUDIOTRACK || hwaout == HWDecoderUtil.AudioOutput.OPENSLES) aout =
            if (hwaout == HWDecoderUtil.AudioOutput.OPENSLES) AOUT_OPENSLES else AOUT_AUDIOTRACK
    ...
    fun setMediaOptions(media: IMedia, context: Context, flags: Int, hasRenderer: Boolean) {
        val noHardwareAcceleration = flags and 0x02 != 0
...
//        val paused = flags and MediaWrapper.MEDIA_PAUSED != 0
        val hardwareAcceleration = HW_ACCELERATION_DISABLED
        val prefs = Settings.getInstance(context)

//        //后续如需切换解码方式，需要验证主板支持能力
//        if (!noHardwareAcceleration) {
//            try {
//                hardwareAcceleration = Integer.parseInt(prefs.getString("hardware_acceleration", "$HW_ACCELERATION_AUTOMATIC")!!)
//            } catch (ignored: NumberFormatException) {}
//        }
        if (hardwareAcceleration == HW_ACCELERATION_DISABLED) media.setHWDecoderEnabled(
            false,
            false
        )
...
//        if (paused) media.addOption(":start-paused")
        ...
        if (hasRenderer) {
            media.addOption(
                ":sout-chromecast-audio-passthrough=" + prefs.getBoolean(
                    "casting_passthrough", true
                )
            )
            media.addOption(
                ":sout-chromecast-conversion-quality=" + prefs.getString(
                    "casting_quality", "2"
                )!!
            )
        }
    ...
    private fun getEqualizerSetFromSettings(pref: SharedPreferences): MediaPlayer.Equalizer? {
        ...
        if (bands != null && pref.contains("equalizer_enabled")) {
            ...
            if (bands.size != bandCount + 1) return null

            val eq = MediaPlayer.Equalizer.create()
            eq.preAmp = bands[0]
            for (i in 0 until bandCount) eq.setAmp(i, bands[i + 1])
            return eq
        } else return MediaPlayer.Equalizer.createFromPreset(0)
    }

    @MainThread
    @JvmOverloads
    fun getEqualizerSetFromSettings(
        context: Context, force: Boolean = false
    ): MediaPlayer.Equalizer? {
        val pref = Settings.getInstance(context)
        return if (!force && !pref.getBoolean(
                "equalizer_enabled", false
            )
        ) null else getEqualizerSetFromSettings(pref)
    }
    ...
    @MainThread
    fun saveEqualizerInSettings(
        context: Context, eq: MediaPlayer.Equalizer?, name: String, enabled: Boolean, saved: Boolean
    ) {
    ...
    fun getSoundFontFile(context: Context) =
        File(context.getDir("soundfont", Context.MODE_PRIVATE)!!.path + "/soundfont.sf2")
}
