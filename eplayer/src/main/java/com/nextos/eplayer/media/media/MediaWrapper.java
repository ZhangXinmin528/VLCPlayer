...
import com.nextos.eplayer.media.Tools;
...
//    public abstract Bookmark addBookmark(long time);
     ...
     */
    public MediaWrapper(long id, String mrl, long time, float position, long length, int type, String title, String filename, String artist, String genre, String album, String albumArtist, int width, int height, String artworkURL, int audio, int spu, int trackNumber, int discNumber, long lastModified, long seen, boolean isThumbnailGenerated, int releaseDate, boolean isPresent) {
        super();
        ...
        init(time, position, length, type, null, title, artist, genre, album, albumArtist, width, height, artworkURL != null ? VLCUtil.UriFromMrl(artworkURL).getPath() : null, audio, spu, trackNumber, discNumber, lastModified, seen, isPresent, null);
        ...
        if (type == TYPE_AUDIO) {
            ...
            if (hasArtistMeta) {
                sb.append(artist);
                if (hasAlbumMeta) sb.append(" - ");
            }
            if (hasAlbumMeta) sb.append(album);
        } else if (type == TYPE_VIDEO) {
            Tools.setMediaDescription(this);
        }

        if (sb.length() > 0) mDescription = sb.toString();
        defineType();
    ...
    private void init(IMedia media) {
        ...
        if (media != null) {
            ...
            updateMeta(media);
            if (mType == TYPE_ALL) switch (media.getType()) {
    ...
    private void defineType() {
        if (mType != TYPE_ALL) return;

        String fileExt = null, filename = mUri.getLastPathSegment();
        if (TextUtils.isEmpty(filename)) filename = mTitle;
        ...
        if (index != -1) fileExt = filename.substring(index).toLowerCase(Locale.ENGLISH);
    ...
    private void init(long time, float position, long length, int type, Bitmap picture, String title, String artist, String genre, String album, String albumArtist, int width, int height, String artworkURL, int audio, int spu, int trackNumber, int discNumber, long lastModified, long seen, boolean isPresent, IMedia.Slave[] slaves) {
        mFilename = null;
        mTime = time;
        mPosition = position;
        mDisplayTime = time;
    ...
    public MediaWrapper(Uri uri, long time, float position, long length, int type, Bitmap picture, String title, String artist, String genre, String album, String albumArtist, int width, int height, String artworkURL, int audio, int spu, int trackNumber, int discNumber, long lastModified, long seen) {
        mUri = uri;
        init(time, position, length, type, picture, title, artist, genre, album, albumArtist, width, height, artworkURL, audio, spu, trackNumber, discNumber, lastModified, seen, true, null);
    }
    ...
    public String getFileName() {
        if (mFilename == null) {
            if (mUri == null) mFilename = "";
            else mFilename = mUri.getLastPathSegment();
        }
        return mFilename;
    }
    ...
    public boolean isPodcast() {
        return mType == TYPE_AUDIO && (mLength > PODCAST_ABSOLUTE || TextUtils.isEmpty(mAlbum) && mLength > PODCAST_THRESHOLD || "podcast".equalsIgnoreCase(mGenre) || "audiobooks".equalsIgnoreCase(mGenre) || "audiobook".equalsIgnoreCase(mGenre) || "speech".equalsIgnoreCase(mGenre) || "vocal".equalsIgnoreCase(mGenre));
    }
    ...
    public MediaWrapper(Parcel in) {
        ...
        init(in.readLong(), in.readFloat(), in.readLong(), in.readInt(), (Bitmap) in.readParcelable(Bitmap.class.getClassLoader()), in.readString(), in.readString(), in.readString(), in.readString(), in.readString(), in.readInt(), in.readInt(), in.readString(), in.readInt(), in.readInt(), in.readInt(), in.readInt(), in.readLong(), in.readLong(), in.readInt() == 1, in.createTypedArray(PSlave.CREATOR));
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        } else dest.writeTypedArray(null, flags);
    }
