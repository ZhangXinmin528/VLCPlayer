...
class PlayerController(val context: Context) : IVLCVout.Callback, MediaPlayer.EventListener,
    ...
    val progress by lazy(LazyThreadSafetyMode.NONE) {
        MutableLiveData<Progress>().apply {
            value = Progress()
        }
    }
    ...
    fun canDoPassthrough() =
        mediaplayer.hasMedia() && !mediaplayer.isReleased && mediaplayer.canDoPassthrough()
    ...
    internal suspend fun startPlayback(
        media: IMedia, listener: MediaPlayerEventListener, time: Long
    ) {
        mediaplayerEventListener = listener
        resetPlaybackState(time, media.duration)
        mediaplayer.setEventListener(null)
        withContext(Dispatchers.IO) {
            if (!mediaplayer.isReleased) mediaplayer.media =
                media.apply { if (hasRenderer) parse() }
        }
    ...
    fun setPosition(position: Float) {
        if (seekable && mediaplayer.hasMedia() && !mediaplayer.isReleased) mediaplayer.position =
            position
    }

    fun setTime(time: Long, fast: Boolean = false) {
        if (seekable && mediaplayer.hasMedia() && !mediaplayer.isReleased) mediaplayer.setTime(
            time, fast
        )
    }
    ...
    fun getAudioDelay() =
        if (mediaplayer.hasMedia() && !mediaplayer.isReleased) mediaplayer.audioDelay else 0L

    fun getSpuDelay() =
        if (mediaplayer.hasMedia() && !mediaplayer.isReleased) mediaplayer.spuDelay else 0L
        ...
        launch(Dispatchers.IO) {
            if (BuildConfig.DEBUG) { // Warn if player release is blocking
                try {
                    ...
                    launch {
                        Toast.makeText(context, "media stop has timeouted!", Toast.LENGTH_LONG)
                            .show()
                    }
                }
            } else player.release()
    ...
    fun setRate(rate: Float, save: Boolean) {
        ...
        if (save && settings.getBoolean(
            ...
            if (isVideoPlaying()) KEY_PLAYBACK_RATE_VIDEO else KEY_PLAYBACK_RATE, rate
        )
    }
    ...
    fun getTitles(): Array<out MediaPlayer.Title>? =
        if (!mediaplayer.isReleased) mediaplayer.titles else emptyArray()
    ...
    private val eventActor = actor<MediaPlayer.Event>(
        capacity = Channel.UNLIMITED, start = CoroutineStart.UNDISPATCHED
    ) {
    ...
    @JvmOverloads
    fun updateProgress(
        newTime: Long = progress.value?.time ?: 0L, newLength: Long = progress.value?.length ?: 0L
    ) {
