...
import com.nextos.module.playermodule.media.MediaWrapperList
import com.nextos.module.playermodule.resources.AppContextProvider
import com.nextos.module.playermodule.resources.EXIT_PLAYER
import com.nextos.module.playermodule.resources.KEY_AUDIO_LAST_PLAYLIST
import com.nextos.module.playermodule.resources.KEY_CURRENT_AUDIO
import com.nextos.module.playermodule.resources.KEY_CURRENT_AUDIO_RESUME_ARTIST
import com.nextos.module.playermodule.resources.KEY_CURRENT_AUDIO_RESUME_THUMB
import com.nextos.module.playermodule.resources.KEY_CURRENT_AUDIO_RESUME_TITLE
import com.nextos.module.playermodule.resources.KEY_CURRENT_MEDIA
import com.nextos.module.playermodule.resources.KEY_CURRENT_MEDIA_RESUME
import com.nextos.module.playermodule.resources.KEY_MEDIA_LAST_PLAYLIST
import com.nextos.module.playermodule.resources.KEY_MEDIA_LAST_PLAYLIST_RESUME
import com.nextos.module.playermodule.resources.PLAYLIST_TYPE_AUDIO
import com.nextos.module.playermodule.resources.PLAYLIST_TYPE_VIDEO
...
import com.nextos.module.playermodule.tools.AUDIO_DELAY_GLOBAL
import com.nextos.module.playermodule.tools.AUDIO_RESUME_PLAYBACK
import com.nextos.module.playermodule.tools.AUDIO_SHUFFLING
import com.nextos.module.playermodule.tools.AUDIO_STOP_AFTER
import com.nextos.module.playermodule.tools.AppScope
import com.nextos.module.playermodule.tools.KEY_AUDIO_FORCE_SHUFFLE
import com.nextos.module.playermodule.tools.KEY_PLAYBACK_RATE
import com.nextos.module.playermodule.tools.KEY_PLAYBACK_RATE_VIDEO
import com.nextos.module.playermodule.tools.KEY_PLAYBACK_SPEED_PERSIST
import com.nextos.module.playermodule.tools.KEY_PLAYBACK_SPEED_PERSIST_VIDEO
import com.nextos.module.playermodule.tools.KEY_VIDEO_CONFIRM_RESUME
import com.nextos.module.playermodule.tools.MEDIA_SHUFFLING
import com.nextos.module.playermodule.tools.PLAYBACK_HISTORY
import com.nextos.module.playermodule.tools.POSITION_IN_AUDIO_LIST
import com.nextos.module.playermodule.tools.POSITION_IN_MEDIA
import com.nextos.module.playermodule.tools.POSITION_IN_MEDIA_LIST
import com.nextos.module.playermodule.tools.POSITION_IN_SONG
import com.nextos.module.playermodule.tools.Settings
import com.nextos.module.playermodule.tools.VIDEO_PAUSED
import com.nextos.module.playermodule.tools.VIDEO_RESUME_PLAYBACK
import com.nextos.module.playermodule.tools.VIDEO_SPEED
import com.nextos.module.playermodule.tools.putSingle
import com.nextos.module.playermodule.util.AndroidDevices
import com.nextos.module.playermodule.util.FileUtils
import com.nextos.module.playermodule.util.isSchemeFD
...
class PlaylistManager(val service: PlaybackService) : MediaWrapperList.EventListener,
    IMedia.EventListener, CoroutineScope {
    ...
    val abRepeat by lazy(LazyThreadSafetyMode.NONE) {
        MutableLiveData<ABRepeat>().apply {
            value = ABRepeat()
        }
    }
    val abRepeatOn by lazy(LazyThreadSafetyMode.NONE) {
        MutableLiveData<Boolean>().apply {
            value = false
        }
    }
    val videoStatsOn by lazy(LazyThreadSafetyMode.NONE) {
        MutableLiveData<Boolean>().apply {
            value = false
        }
    }
    val delayValue by lazy(LazyThreadSafetyMode.NONE) {
        MutableLiveData<DelayValues>().apply {
            value = DelayValues()
        }
    }
    val waitForConfirmation by lazy(LazyThreadSafetyMode.NONE) {
        MutableLiveData<WaitConfirmation?>().apply {
            value = null
        }
    }
    ...
    fun resetResumeStatus() {
        ...
        videoResumeStatus =
            if (string == "2") VideoResumeStatus.ASK else if (string == "0") VideoResumeStatus.ALWAYS else VideoResumeStatus.NEVER
    ...
    suspend fun load(
        ...
        if (settings.getBoolean(
                KEY_AUDIO_FORCE_SHUFFLE, false
            ) && getCurrentMedia()?.type == MediaWrapper.TYPE_AUDIO && !shuffling && canShuffle()
        ) shuffle()
    }
    ...
    fun loadLastPlaylist(type: Int = PLAYLIST_TYPE_AUDIO): Boolean {
        ...
        val locations = settings.getString(locationsKey, null)?.split(" ".toRegex())
        ...
        launch {
            ...
            val position = max(
                0, settings.getInt(if (audio) POSITION_IN_AUDIO_LIST else POSITION_IN_MEDIA_LIST, 0)
            )
    ...
    fun stop(systemExit: Boolean = false, video: Boolean = false) {
        clearABRepeat()
        if (stopAfter != -1) Settings.getInstance(AppContextProvider.appContext)
            .putSingle(AUDIO_STOP_AFTER, stopAfter)
        stopAfter = -1
        videoBackground = false
    ...
    fun previous(force: Boolean) {
        mediaList.getMedia(currentIndex)
        if (hasPrevious() && ((force || !player.seekable || (player.getCurrentTime() < PREVIOUS_LIMIT_DELAY) || (lastPrevious != -1L && System.currentTimeMillis() - lastPrevious < PREVIOUS_LIMIT_DELAY)))) {
            val size = mediaList.size()
    ...
    private fun setRepeatTypeFromSettings() {
        ...
        } else settings.getInt(PLAYLIST_AUDIO_REPEAT_MODE_KEY, PlaybackStateCompat.REPEAT_MODE_NONE)
    }

    @MainThread
    fun setRepeatType(repeatType: Int) {
        repeating = repeatType
        if (getCurrentMedia()?.type == MediaWrapper.TYPE_VIDEO) settings.putSingle(
            PLAYLIST_VIDEO_REPEAT_MODE_KEY, repeating
        )
        else settings.putSingle(PLAYLIST_AUDIO_REPEAT_MODE_KEY, repeating)
        savePosition()
        launch { determinePrevAndNextIndices() }
    }
    ...
    fun setRenderer(item: RendererItem?) {
        ...
        showAudioPlayer.value =
            PlayerController.playbackState != PlaybackStateCompat.STATE_STOPPED && (item !== null || !player.isVideoPlaying())
    }

    suspend fun playIndex(
        index: Int, flags: Int = 0, forceResume: Boolean = false, forceRestart: Boolean = false
    ) {
        ...
        if (mw.type != MediaWrapper.TYPE_VIDEO || isVideoPlaying || player.hasRenderer || mw.hasFlag(
            ...
            val start =
                if (forceRestart || videoResumeStatus == VideoResumeStatus.NEVER) 0L else getStartTime(
                    mw
                )
            if (isVideoPlaying) {
            ...
            VLCOptions.setMediaOptions(
                media, ctx, flags or mw.flags, PlaybackService.hasRenderer()
    ...
    fun switchToVideo(): Boolean {
        ...
        if (media === null || media.hasFlag(MediaWrapper.MEDIA_FORCE_AUDIO) || !player.canSwitchToVideo()) return false
    ...
    fun setSpuTrack(index: String) {
        ...
        if (media.id != 0L) launch(Dispatchers.IO) {
            media.setStringMeta(
                MediaWrapper.META_SUBTITLE_TRACK, index
            )
        }
    }
    ...
    fun setAudioDelay(delay: Long) {
        ...
        if (media.id != 0L && settings.getBoolean("save_individual_audio_delay", true)) {
            launch(Dispatchers.IO) {
                media.setLongMeta(
                    MediaWrapper.META_AUDIODELAY, player.getAudioDelay()
                )
            }
        }
    }
    ...
    fun setSpuDelay(delay: Long) {
        ...
        if (media.id != 0L) launch(Dispatchers.IO) {
            media.setLongMeta(
                MediaWrapper.META_SUBTITLE_DELAY, player.getSpuDelay()
            )
        }
    }
    ...
    private fun loadMediaMeta(media: MediaWrapper) {
        if (player.canSwitchToVideo()) {
            ...
            val globalDelay =
                Settings.getInstance(AppContextProvider.appContext).getLong(AUDIO_DELAY_GLOBAL, 0L)
    ...
    override fun onItemMoved(indexBefore: Int, indexAfter: Int, mrl: String) {
        ...
        when (currentIndex) {
            indexBefore -> {
                ...
                if (indexAfter > indexBefore) --currentIndex
            }
    ...
    fun previousTotalTime(): Long {
        ...
        return when {
            ...
            shuffling -> {
                copy.asSequence().filterIndexed { prevIndex, _ -> previous.contains(prevIndex) }
                    .map { it.length }.sum()
            }

            else -> {
                copy.asSequence().take(index).map { it.length }.sum()
            }
        }
    }
    ...
    private suspend fun expand(updateHistory: Boolean): Int {
        entryUrl = null
        Log.d(TAG, "expand with values: ", Exception("Call stack"))
        val index = currentIndex
    ...
    private fun savePosition(reset: Boolean = false, video: Boolean = false) {
        ...
        settings.edit {
            ...
            putInt(
                if (audio) POSITION_IN_AUDIO_LIST else POSITION_IN_MEDIA_LIST,
                if (reset) 0 else currentIndex
            )
            putLong(
                if (audio) POSITION_IN_SONG else POSITION_IN_MEDIA,
                if (reset) 0L else player.getCurrentTime()
            )
            if (!audio) {
            ...
            if (reset) {
                ...
                if (audio && settings.getBoolean(AUDIO_RESUME_PLAYBACK, true)) putString(
                    KEY_CURRENT_AUDIO, media.location
                )
            }
        }
    }
...
//        ) shuffle()
    ...
    fun insertNext(list: List<MediaWrapper>) {
        Log.d(TAG, "insertNext with values: ", Exception("Call stack"))
    ...
    fun insertItem(position: Int, mw: MediaWrapper) {
        Log.d(TAG, "insertItem with values: ", Exception("Call stack"))
        Log.d(TAG, "Media location: ${mw.uri}")
    ...
    override fun onEvent(event: IMedia.Event) {
        var update = true
        when (event.type) {
            IMedia.Event.MetaChanged -> {/* Update Meta if file is already parsed */
                if (parsed && player.updateCurrentMeta(
                        event.metaId, getCurrentMedia()
                    )
                ) service.onMediaListChanged()
                Log.i(TAG, "Media.Event.MetaChanged: " + event.metaId)
            }
            ...
            when (event.type) {
                MediaPlayer.Event.Playing -> {
                    ...
                    if (newMedia) {
                        ...
                        saveCurrentMedia()
                        newMedia = false
                        if (player.hasRenderer || !player.isVideoPlaying()) showAudioPlayer.value =
                            true
                        savePlaycount(mw)
                        if (mw.title == mw.fileName || (mw.type == MediaWrapper.TYPE_STREAM && (mw.title != player.mediaplayer.media?.getMeta(
                                IMedia.Meta.Title, true
                            ) || mw.artist != player.mediaplayer.media?.getMeta(
                                IMedia.Meta.Artist, true
                            )))
                        ) {
                ...
                MediaPlayer.Event.EncounteredError -> {
                    ...
                    service.showToast(
                        if (location != null && Uri.parse(location).scheme == "missing") service.getString(
                            R.string.missing_location
                        ) else service.getString(
                            R.string.invalid_location, location ?: ""
                ...
                MediaPlayer.Event.SeekableChanged -> if (event.seekable && settings.getBoolean(
                        if (player.isVideoPlaying()) KEY_PLAYBACK_SPEED_PERSIST_VIDEO else KEY_PLAYBACK_SPEED_PERSIST,
                        false
                    )
                ) {
                    player.setRate(
                        settings.getFloat(
                            if (player.isVideoPlaying()) KEY_PLAYBACK_RATE_VIDEO else KEY_PLAYBACK_RATE,
                            1.0f
                        ), false
                    )
                }
                ...
                MediaPlayer.Event.ESSelected -> {
                    getCurrentMedia()?.let { media ->
                        if (player.isPlaying()) {
                            if (media.type == MediaWrapper.TYPE_STREAM && (media.title != player.mediaplayer.media?.getMeta(
                                    IMedia.Meta.Title, true
                                ) || media.artist != player.mediaplayer.media?.getMeta(
                                    IMedia.Meta.Artist, true
                                ))
                            ) {
    ...
    private suspend fun savePlaycount(mw: MediaWrapper) {
        var currentMedia = mw
        if (settings.getBoolean(PLAYBACK_HISTORY, true) && !mw.uri.scheme.isSchemeFD()) withContext(
            Dispatchers.IO
        ) {
            var id = mw.id
