...
//            hasAllAccess(context)
    ...
    private fun createDialog(activity: FragmentActivity, exit: Boolean): Dialog {
        val dialogBuilder = android.app.AlertDialog.Builder(activity)
                .setTitle(activity.getString(R.string.player_allow_storage_access_title))
                .setMessage(activity.getString(R.string.player_allow_storage_access_description))
//                .setIcon(R.drawable.ic_warning)
                .setPositiveButton(activity.getString(R.string.player_permission_ask_again)) { _, _ ->
                    val settings = Settings.getInstance(activity)
                        activity.requestStoragePermission()
                    settings.putSingle("user_declined_storage_access", true)
                }
        if (exit) {
            dialogBuilder.setNegativeButton(activity.getString(R.string.player_exit_app)) { _, _ -> activity.finish() }
                    .setCancelable(false)
        }
    ...
    private fun createExternalManagerDialog(activity: FragmentActivity, listener: (boolean: Boolean) -> Unit): Dialog {
        val dialogBuilder = android.app.AlertDialog.Builder(activity)
                .setTitle(activity.getString(R.string.player_allow_storage_manager_title))
                .setMessage(activity.getString(R.string.player_allow_storage_manager_description, activity.getString(R.string.player_allow_storage_manager_explanation)))
//                .setIcon(R.drawable.ic_warning)
                .setPositiveButton(activity.getString(R.string.player_ok)) { _, _ ->
                    listener.invoke(true)
                }.setNegativeButton(activity.getString(R.string.player_cancel)) { _, _ ->
                    activity.finish()
    ...
    private fun createSettingsDialogCompat(activity: Activity, mode: Int): Dialog {
        ...
        when (mode) {
            PERMISSION_SYSTEM_RINGTONE -> {
                titleId = R.string.player_allow_settings_access_ringtone_title
                textId = R.string.player_allow_settings_access_ringtone_description
            }
            PERMISSION_SYSTEM_BRIGHTNESS -> {
                titleId = R.string.player_allow_settings_access_brightness_title
                textId = R.string.player_allow_settings_access_brightness_description
            }
            PERMISSION_SYSTEM_DRAW_OVRLAYS -> {
                titleId = R.string.player_allow_draw_overlays_title
                textId = R.string.player_allow_sdraw_overlays_description
                action = android.provider.Settings.ACTION_MANAGE_OVERLAY_PERMISSION
            }
            PERMISSION_PIP -> {
                titleId = R.string.player_allow_pip
                textId = R.string.player_allow_pip_description
